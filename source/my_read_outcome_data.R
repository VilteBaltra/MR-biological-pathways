# editted read_outcome_data and format_data to include my_log_pval = FALSE and dat$pval <- 10^dat[[pval_col]] 
my_format_data <- function (dat, type = "exposure", snps = NULL, header = TRUE, 
          ...
          info_col = "info", chr_col = "chr", pos_col = "pos", log_pval = FALSE, my_log_pval = FALSE) 
{
  ...
  if (my_log_pval) { # VB: added my_log_pval, for sumstats that have log10(P), instead of -log10(P) (i.e., it's not negative)
    dat$pval <- 10^dat[[pval_col]] 
  }
...
my_read_outcome_data <- function (filename, snps = NULL, sep = " ", phenotype_col = "Phenotype", 
                                  ...
                                  gene_col = "gene", id_col = "id", min_pval = 1e-200, log_pval = FALSE, my_log_pval = FALSE, 
                                  chr_col = "chr", pos_col = "pos") 
{
  outcome_dat <- data.table::fread(filename, header = TRUE, 
                                   sep = sep)
  outcome_dat <- my_format_data(as.data.frame(outcome_dat), type = "outcome", 
