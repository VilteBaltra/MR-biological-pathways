...
# source edited versions of read_outcome_data() and format_data() that include 'my_log_pval' option and dat$pval <- 10^dat[[pval_col]] 
source(paste0(FUNCTIONS, "my_read_outcome_data.R"))
# this version runs on sumstats that have log10(P) instead of -log10(P)

# now make TwoSampleMR package functions available for the edited functions
environment(my_read_outcome_data) <- asNamespace('TwoSampleMR')
environment(my_format_data) <- asNamespace('TwoSampleMR') # my_format_data is inside my_read_outcome_data.R
...
for(trait in file_names){
  ...
  out <- my_read_outcome_data(
    snps = CM_exp_dat$SNP,
    filename = paste0(SUMSTATS, trait, ".tbl.rsid.txt.gz"),
    sep = "\t", snp_col = "rsID",
...
# save all output
openxlsx::write.xlsx(df_a_paths, file = paste0("olink-MM-uniMR-a-paths-", Sys.Date(), ".xlsx"), rowNames = F, overwrite = T)
openxlsx::write.xlsx(egger_intercept_df, file = paste0("olink-egger-intercept-a-paths-", Sys.Date(), ".xlsx"),rowNames = F, overwrite = T)
openxlsx::write.xlsx(Q_df, file = paste0("olink-MM-Q-stat-a-paths-", Sys.Date(), ".xlsx"),rowNames = F, overwrite = T)
...
# save all output
openxlsx::write.xlsx(df, file = paste0("olink-MM-uniMR-unadjusted-b-paths-", Sys.Date(), ".xlsx"),rowNames = F, overwrite = T)
openxlsx::write.xlsx(egger_intercept_df_b, file = paste0("olink-egger-intercept-b-paths-", Sys.Date(), ".xlsx"),rowNames = F, overwrite = T)
openxlsx::write.xlsx(Q_df_b, file = paste0("olink-MM-Q-stat-b-paths-", Sys.Date(), ".xlsx"),rowNames = F, overwrite = T)

# check which IVW estimates are significant
df %>% filter(method == "Inverse variance weighted") %>% filter(pval < 0.05)
# TWEAK-1, SIRT2-1, CCL11-1, CSF.1-1, LIF.R-1, IL.33-1 


#---------------------------------------------#
#     UNIVARIABLE MR for CRP (a path)
#---------------------------------------------#

### CRP ### --> add to inflammation scripts
# CM --> CRP
out_crp <- read_outcome_data(
  snps = CM_exp_dat$SNP,
  filename = paste0("../../../../../summary-stats/CRP-Said-2022/GCST90029070_buildGRCh37.tsv.gz"), # no eaf in sumstats
  sep = "\t", snp_col = "variant_id",
  beta_col = "beta", se_col = "standard_error",
  effect_allele_col = "effect_allele", other_allele_col = "other_allele",
  pval_col = "p_value",  id_col = "CRP",
  chr_col = "chromosome", pos_col = "base_pair_location")

dat_crp <- harmonise_data(
  exposure_dat = CM_exp_dat,
  outcome_dat = out_crp)
dat_crp$outcome <- "CRP"
...
# Horizontal pleiotropy
# The intercept term in MR Egger regression can be a useful indication of whether directional horizontal pleiotropy is driving the results of an MR analysis. 
egger_intercept_crp_a <- mr_pleiotropy_test(dat_crp)

# Heterogeneity statistic
Q_crp_a <- mr_heterogeneity(dat_crp)

# MR
res_crp_a <- mr(dat_crp, method_list = c("mr_ivw", "mr_weighted_median", "mr_egger_regression"))

# combine output into one
res_crp1 <- merge(res_crp_a, Q_crp_a[, c('method', 'Q', 'Q_df', 'Q_pval')], by = 'method', all.x = TRUE)
# and two empty rows to cbind with output
egger_intercept_crp_a[2, ] <- NA
egger_intercept_crp_a[3, ] <- NA
res_crp2 <- cbind(res_crp1, egger_intercept_crp_a[, c('egger_intercept', 'se', 'pval')])
...
crp_dat2 <- harmonise_data(
  exposure_dat = crp_exp_dat_clumped,
  outcome_dat = crp_out2)

# Horizontal pleiotropy
# The intercept term in MR Egger regression can be a useful indication of whether directional horizontal pleiotropy is driving the results of an MR analysis. 
egger_intercept_crp_b <- mr_pleiotropy_test(crp_dat2)

# Heterogeneity statistic
Q_crp_b <- mr_heterogeneity(crp_dat2)

# MR
res_crp_b <- mr(crp_dat2, method_list = c("mr_ivw", "mr_weighted_median", "mr_egger_regression"))
...
openxlsx::write.xlsx(list("a path" = res_crp2, "b path" = res_crp_b2), file = paste0("CM-CRP-univariable-MR-unadjusted-a-and-b-", Sys.Date(), ".xlsx"), rowNames = F, overwrite = T)
